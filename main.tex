...
\begin{frame}
  \makebox[\textwidth][c]{
    \begin{tikzpicture}
      \node (observation) at (0, 4) {Remote sensing measurement};
      \node (product) at (2, 4) {Leaf area index};
      \node (model) at (4, 0) {Model};
      \node (model lai) at (2, 0) {Leaf area index};
      \node (model spectra) at (0, 0) {Modeled spectra};
      \path [->] (observation) edge node [above] {model} (product);
      \path [<->] (product) edge node {} (model lai);
      \path [->] (model) edge node {} (model lai);
      \path [->, bend left] (model) edge node {} (model spectra);
    \end{tikzpicture}
  }
\end{frame}

\begin{frame}
  \frametitle{Scaling from leaf to canopy requires canopy structural parameters}
  \makebox[\textwidth][c]{
    \begin{tikzpicture}[title/.style = {anchor = north, align = center}]
      ...
      \node [title] at ([xshift=-5mm] lai.south) {Leaf area index};
      \node [pic] (clumping closed) at (0, 0) {
        \includegraphics[height = 30mm]{\string ~/Pictures/common_figures/canopy_hemiphoto_closed.jpg}
      };
      \node [pic, anchor = west] (clumping open) at (clumping closed.east) {
        \includegraphics[height = 30mm]{\string ~/Pictures/common_figures/clumped_canopy.png}
      };
      \node [title] at ($(clumping closed.south)!0.5!(clumping open.south)$) {Canopy clumping};
      \node [pic] (leaf angle) at (8, 0) {
        \includegraphics[height = 23mm, clip, trim = {0 5mm 0 0}]{\string ~/Pictures/common_figures/leaf_angle_conceptual.png}
      };
      \node [title] at (leaf angle.south) {Leaf angle distribution};
    \end{tikzpicture}
  }
\end{frame}
